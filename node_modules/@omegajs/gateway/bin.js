...
if (isBootstrap) {
  const port = Number(arg('port') || '0') || 1776
  const host = arg('host')
...
async function startBootstrapNode (port, host) {
  ...
  const node = OGateway.bootstrapper(port, host)
  await node.ready()
  ...
  console.log('Fully started oGateway DHT bootstrap node')
...
async function startNodes (cnt, bootstrap) {
  ...
  while (all.length < cnt) {
    const node = new OGateway({ host, port, anyPort: !port, bootstrap })
    await node.ready()
  ...
  console.log('Fully started ' + cnt + ' oGateway DHT node' + (cnt === 1 ? '' : 's'))
