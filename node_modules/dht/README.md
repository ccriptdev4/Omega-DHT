...
```sh
npm install -g bitdht
```

Run a DHT node:
```sh
bitdht # [--port 0] [--host 0.0.0.0] [--bootstrap <comma separated list of ip:port>]
```

Or run multiple nodes:
```sh
bitdht --nodes 5 # [--host 0.0.0.0] [--bootstrap <list>]
```
...
```sh
bitdht --bootstrap --host (server-ip) # [--port 49737]
```
...
```sh
bitdht --port 49738 --bootstrap (server-ip):49737
```
